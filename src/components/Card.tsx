...
export function Card({ card, onClick, disabled, cols }: CardProps) {
  ...
  return (
    <div className="card-3d cursor-pointer select-none" onClick={handleClick}>
      <motion.div
        className="relative w-full"
        whileTap={{ scale: isRevealed ? 1 : 0.94 }}
        transition={{ type: 'spring', stiffness: 400, damping: 20 }}
      >
        <div className={`card-inner relative w-full ${sizeClass} ${isRevealed ? 'card-flipped' : ''}`}>
          {/* Back face */}
          <div className="card-face absolute inset-0 rounded-xl bg-gradient-to-br from-zen-500 to-zen-600 flex items-center justify-center shadow-lg shadow-teal-900/40 border border-teal-400/30">
            <span className="font-display text-lg font-bold text-teal-50/90">বাংলা</span>
          </div>
